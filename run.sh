...
function push-initial-readme-to-repo {
    rm -rf "$REPO_NAME"
    gh repo clone "octaviodeliberato/$REPO_NAME"
    cd "$REPO_NAME"
    ...
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://octaviodeliberato:$GH_TOKEN@github.com/octaviodeliberato/$REPO_NAME"
    fi
...
function create-repo-if-not-exists {
    ...
    echo "Checking to see if octaviodeliberato/$REPO_NAME exists..."
    gh repo view "octaviodeliberato/$REPO_NAME" > /dev/null \
        && echo "Repo exists" \
        && exit 0
    ...
    gh repo create "octaviodeliberato/$REPO_NAME" "--$PUBLIC_OR_PRIVATE"
...
function configure-repo {
    ...
    gh secret set TEST_PYPI_TOKEN \
        ...
        --repo "octaviodeliberato/$REPO_NAME"
    ...
    BRANCH_NAME="main"
    gh api -X PUT "repos/octaviodeliberato/$REPO_NAME/branches/$BRANCH_NAME/protection" \
...
EOF
    ...
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://octaviodeliberato:$GH_TOKEN@github.com/phitoduck/$REPO_NAME"
    fi
